# Schema Templates Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add one-click predefined extraction schemas (factura, contrato, recibo, DNI) to the SchemaEditor component so users don't have to define fields manually.

**Architecture:** Pure UI change — template definitions live as a constant inside `SchemaEditor.tsx`. A dropdown button opens a small popover listing template names; selecting one replaces (or appends to) the current schema. No new files, no DB changes, no type changes.
...
| File | Action |
|------|--------|
| `src/components/documents/SchemaEditor.tsx` | Modify — add `SCHEMA_TEMPLATES` constant + template picker UI |

---

### Task 1: Add template definitions and picker UI to SchemaEditor

**Files:**
- Modify: `src/components/documents/SchemaEditor.tsx`

Current file location: `/home/user/transformerjs-demo/src/components/documents/SchemaEditor.tsx`

The full current file content (read it first, then apply the diff below):

```tsx
// Current SchemaEditor has: slugify(), SchemaEditor component with addField, removeField, onChange
// Props: { schema: ExtractionField[], onChange: (schema: ExtractionField[]) => void }
```

- [ ] **Step 1: Read the current SchemaEditor**

```bash
cat src/components/documents/SchemaEditor.tsx
```

- [ ] **Step 2: Add SCHEMA_TEMPLATES constant and template picker state**

Replace the entire `SchemaEditor.tsx` with the following (preserving all existing logic, adding templates on top):
...
- [ ] **Step 4: Close template popover on outside click (add useEffect)**

Open `src/components/documents/SchemaEditor.tsx` and add a click-outside handler. After the `const [templateOpen, setTemplateOpen] = useState(false)` line, add:
...
Then wrap the relative div with `ref={templateRef}`:
```tsx
...
2. Open a document, go to "Datos estructurados"
3. Click "Usar plantilla" — popover with 4 options appears
4. Click "Factura" — schema is populated with 5 fields
5. Click outside — popover closes
6. Click "Usar plantilla" > "DNI / Pasaporte" — schema replaces with 5 DNI fields
7. Click "Guardar schema" — schema persists on reload

- [ ] **Step 7: Commit**

```bash
cd /home/user/transformerjs-demo
git add src/components/documents/SchemaEditor.tsx
git commit -m "feat: predefined extraction schema templates (factura, contrato, recibo, DNI)"
